    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          setDoc(docRef, { count: 200, lastVisited: serverTimestamp() }, { merge: true })
            .then(() => {
              setCount(200)
              localStorage.setItem('cached_global_visitor_count', '200')
            })
      ...
      (err) => {
        ...
        const stored = localStorage.getItem('cached_global_visitor_count')
        let currentCount = stored ? parseInt(stored, 10) : 200

        // Only increment fallback count ONCE per session
        if (!hasCounted) {
          sessionStorage.setItem(SESSION_KEY, 'true')
          currentCount += 1
          localStorage.setItem('cached_global_visitor_count', String(currentCount))
        }
