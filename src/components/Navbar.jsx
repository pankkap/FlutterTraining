      ...
      <nav className="page-shell flex items-center justify-between py-3">
        <a href="#top" className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="text-[10px] font-semibold text-indigo-600 uppercase tracking-widest block">Industrial Training </span>
          </div>
        </a>
        <div className="hidden items-center gap-8 lg:flex">
          {[
            ...
            { href: '#pricing', label: 'Pricing & Admission' },
            { href: '#faq', label: 'FAQ' },
          ].map((link) => (
            <a
              key={link.href}
        ...
        <CTAButton label="Enroll Now — Rs 7,500" onClick={onReserve} className="py-2.5 px-5 text-xs sm:text-sm" />
      </nav>
    </header>
  )
}
